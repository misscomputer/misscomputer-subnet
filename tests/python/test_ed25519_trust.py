...
def test_complete_small_order_encoding_set_is_rejected(
    ...
    with pytest.raises(Ed25519PublicKeyValidationError) as hex_caught:
        decode_ed25519_public_key_hex(small_order_ed25519_public_key.hex())
    assert hex_caught.value.code == "ed25519_public_key_small_order"
    assert str(hex_caught.value) == "ed25519_public_key_small_order"
...
@pytest.mark.parametrize(
    "value",
    ["", "zz" * 32, "00" * 31, "00" * 33, "AB" * 32],
)
def test_noncanonical_or_wrong_length_hex_public_key_is_rejected(value: str) -> None:
    with pytest.raises(Ed25519PublicKeyValidationError) as caught:
        decode_ed25519_public_key_hex(value)
    assert caught.value.code == "ed25519_public_key_encoding_invalid"
    assert str(caught.value) == "ed25519_public_key_encoding_invalid"
...
def test_representative_valid_public_keys_are_preserved(public_hex: str) -> None:
    ...
    assert decode_ed25519_public_key_hex(public_hex) == public_bytes
